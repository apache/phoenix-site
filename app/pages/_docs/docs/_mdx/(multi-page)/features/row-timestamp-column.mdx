...
The [`PHOENIX_ROW_TIMESTAMP()`](/docs/features/phoenix-row-timestamp) function lets you read the underlying HBase row timestamp on any Phoenix table — even one without a declared `ROW_TIMESTAMP` column — and is indexable for fast time-bounded queries.
