import{j as e}from"./jsx-runtime-fm7E3NsZ.js";let a=`To support atomic upsert, an optional \`ON DUPLICATE KEY\` clause, similar to the MySQL syntax, has been
...
* \`executeAtomicUpdateReturnRow\` — returns the row state **after** the mutation
  (or the unchanged row when the statement was a no-op). Equivalent to
  \`RETURNING *\` in SQL.
* \`executeAtomicUpdateReturnOldRow\` — returns the row state **before** the
  mutation, regardless of whether the mutation was applied. There is no SQL form
  for this — only the typed API exposes the pre-image, which is the natural fit
  for audit logs and compare-and-swap-style flows.
...
`,h={title:"Atomic Upsert",description:"Use UPSERT ... ON DUPLICATE KEY for atomic row-level updates in Phoenix, with examples and limitations."},r=[{href:"/docs/grammar#upsert-values"},{href:"https://issues.apache.org/jira/browse/PHOENIX-7648"},{href:"https://issues.apache.org/jira/browse/PHOENIX-7651"}],l={contents:[{heading:void 0,content:`To support atomic upsert, an optional ON DUPLICATE KEY clause, similar to the MySQL syntax, has been
incorporated into the UPSERT VALUES command as of Phoenix 4.9. The general syntax is described
...
exists — it never inserts. If the row is missing, the supplied VALUES are discarded
and the statement is a no-op. Use it when an UPSERT should be able to update an
existing row but must not create a new one.`},{heading:"atomic-upsert-update-only",content:`If id = 'sku-42' already exists, its qty is decremented under the row lock.
...
the returned ResultSet exactly like the result of a regular SELECT.`},{heading:"java-jdbc-thick-client-api",content:`Java applications using the Phoenix thick JDBC client can drive the same
atomic-return semantics directly through typed methods on PhoenixStatement and
PhoenixPreparedStatement, without putting RETURNING * in the SQL:`},{heading:"java-jdbc-thick-client-api",content:`executeAtomicUpdateReturnRow — returns the row state after the mutation
(or the unchanged row when the statement was a no-op). Equivalent to
RETURNING * in SQL.`},{heading:"java-jdbc-thick-client-api",content:`executeAtomicUpdateReturnOldRow — returns the row state before the
mutation, regardless of whether the mutation was applied. There is no SQL form
for this — only the typed API exposes the pre-image, which is the natural fit
for audit logs and compare-and-swap-style flows.`},{heading:"java-jdbc-thick-client-api",content:`Both return a Pair<Integer, ResultSet> where the integer is 1 if the row was
mutated and 0 for a no-op, so callers know the outcome without having to inspect
cell contents. Both require an auto-commit connection.`},{heading:"atomic-upsert-limitations",content:"The following limitations are enforced for the ON DUPLICATE KEY clause usage:"},{heading:"atomic-upsert-limitations",content:"Primary key columns may not be updated, since this would essentially be creating a new row."},{heading:"atomic-upsert-limitations",content:`Transactional tables may not use this clause as atomic upserts are already possible through
...
does not handle atomicity unless the latest value is being updated.`},{heading:"atomic-upsert-limitations",content:"The same column should not be updated more than once in the same statement."},{heading:"atomic-upsert-limitations",content:"No aggregation or references to sequences are allowed within the clause."}],headings:[{id:"atomic-upsert-examples",content:"Examples"},{id:"atomic-upsert-update-only",content:"ON DUPLICATE KEY UPDATE_ONLY"},{id:"atomic-upsert-returning",content:"Returning the affected row"},{id:"java-jdbc-thick-client-api",content:"Java JDBC thick-client API"},{id:"atomic-upsert-limitations",content:"Limitations"}]};const d=[{depth:2,url:"#atomic-upsert-examples",title:e.jsx(e.Fragment,{children:"Examples"})},{depth:2,url:"#atomic-upsert-update-only",title:e.jsx(e.Fragment,{children:"ON DUPLICATE KEY UPDATE_ONLY"})},{depth:2,url:"#atomic-upsert-returning",title:e.jsx(e.Fragment,{children:"Returning the affected row"})},{depth:3,url:"#java-jdbc-thick-client-api",title:e.jsx(e.Fragment,{children:"Java JDBC thick-client API"})},{depth:2,url:"#atomic-upsert-limitations",title:e.jsx(e.Fragment,{children:"Limitations"})}];function t(s){const i={a:"a",code:"code",em:"em",h2:"h2",h3:"h3",li:"li",p:"p",pre:"pre",span:"span",strong:"strong",table:"table",tbody:"tbody",td:"td",th:"th",thead:"thead",tr:"tr",ul:"ul",...s.components};return e.jsxs(e.Fragment,{children:[e.jsxs(i.p,{children:["To support atomic upsert, an optional ",e.jsx(i.code,{children:"ON DUPLICATE KEY"}),` clause, similar to the MySQL syntax, has been
...
Unlike `,e.jsx(i.code,{children:"UPDATE"}),", it ",e.jsx(i.strong,{children:"only"}),` runs the update-clause expressions when the row already
exists — it never inserts. If the row is missing, the supplied `,e.jsx(i.code,{children:"VALUES"}),` are discarded
and the statement is a no-op. Use it when an `,e.jsx(i.code,{children:"UPSERT"}),` should be able to update an
existing row but must not create a new one.`]}),`
...
`,e.jsx(i.h3,{id:"java-jdbc-thick-client-api",children:"Java JDBC thick-client API"}),`
`,e.jsxs(i.p,{children:[`Java applications using the Phoenix thick JDBC client can drive the same
atomic-return semantics directly through typed methods on `,e.jsx(i.code,{children:"PhoenixStatement"}),` and
`,e.jsx(i.code,{children:"PhoenixPreparedStatement"}),", without putting ",e.jsx(i.code,{children:"RETURNING *"})," in the SQL:"]}),`
`,e.jsxs(i.ul,{children:[`
`,e.jsxs(i.li,{children:[e.jsx(i.code,{children:"executeAtomicUpdateReturnRow"})," — returns the row state ",e.jsx(i.strong,{children:"after"}),` the mutation
(or the unchanged row when the statement was a no-op). Equivalent to
`,e.jsx(i.code,{children:"RETURNING *"})," in SQL."]}),`
`,e.jsxs(i.li,{children:[e.jsx(i.code,{children:"executeAtomicUpdateReturnOldRow"})," — returns the row state ",e.jsx(i.strong,{children:"before"}),` the
mutation, regardless of whether the mutation was applied. There is no SQL form
for this — only the typed API exposes the pre-image, which is the natural fit
for audit logs and compare-and-swap-style flows.`]}),`
`]}),`
`,e.jsxs(i.p,{children:["Both return a ",e.jsx(i.code,{children:"Pair<Integer, ResultSet>"})," where the integer is ",e.jsx(i.code,{children:"1"}),` if the row was
mutated and `,e.jsx(i.code,{children:"0"}),` for a no-op, so callers know the outcome without having to inspect
cell contents. Both require an auto-commit connection.`]}),`
...
`]})]})}function o(s={}){const{wrapper:i}=s.components||{};return i?e.jsx(i,{...s,children:e.jsx(t,{...s})}):t(s)}export{a as _markdown,o as default,r as extractedReferences,h as frontmatter,l as structuredData,d as toc};
