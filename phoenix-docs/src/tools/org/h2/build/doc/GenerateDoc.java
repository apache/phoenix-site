...
public class GenerateDoc {
    ...
    private String inHelp = "src/docsrc/help/phoenix.csv";
    ...
    private void run(String... args) throws Exception {
        ...
        bnf = Bnf.getInstance(new FileReader(inHelp));
        bnf.linkStatements();
    ...
    private void map(String key, String sql, boolean railroads) throws Exception {
        ...
        try {
            ...
            int div = Math.min(3,Math.max(1,list.size()));
            ...
            for (int i = list.size(); i < 3; i++) {
                List<HashMap<String, String>> listThird = Collections.emptyList();
                session.put(key + "-" + i, listThird);
            }
        } finally {
